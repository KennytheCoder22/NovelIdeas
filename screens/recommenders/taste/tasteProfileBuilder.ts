...
// refine it with recommendation feedback and star ratings.
...
function feedbackScalar(event: TasteFeedbackEvent): number {
  ...
  if (event.kind === "already_read") {
    ...
    // Map 1..5 to roughly -1..1, with a little extra strength for true ratings.
...
export function normalizeTasteProfile(profile: TasteProfile): TasteProfile {
  ...
  for (const axis of TASTE_AXES) {
    // tanh-like squashing without requiring Math.tanh in every runtime target.
...
export function buildTasteProfile(input: TasteBuilderInput): TasteProfile {
  ...
  return applyFeedbackToTasteProfile(base, input.feedback || [], input.itemTraitsById || {});
}
