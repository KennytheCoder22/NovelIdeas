...
// PATCHED: preserve Open Library subject: syntax while still extracting free-text terms
function extractQueryTerms(queryText: any): string[] {
  const raw = String(queryText || '').toLowerCase();

  if (!raw.trim()) return [];

  // Capture subject tokens like:
  // subject:science_fiction OR subject:"science fiction"
  ...
  // Remove subject tokens before free-text parsing
