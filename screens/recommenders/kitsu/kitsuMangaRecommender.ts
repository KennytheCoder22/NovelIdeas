...
function buildKitsuQueries(input: RecommenderInput): string[] {
  ...
  const anchors = [
    ...(brief.thematicAnchors || []),
    ...(brief.genreAnchors || []),
    ...((((brief as any).enforcedNonGenre) || []) as string[]),
    brief.toneAnchor || "",
  ]
  ...
  for (const v of anchors) {
    if (v.includes("psychological") || v.includes("mystery")) add("psychological");
    if (v.includes("psychological") || v.includes("mystery")) add("mystery");
    if (v.includes("speculative") || v.includes("science")) add("science fiction");
    if (v.includes("adventure") || v.includes("survival")) add("adventure");
    if (v.includes("adventure") || v.includes("survival") || v.includes("thriller")) add("action");
    if (v.includes("dark")) add("horror");
    if (v.includes("superhero")) add("superhero");
    if (v.includes("coming of age") || v.includes("identity") || v.includes("found family")) add("shonen");
  }
...
export async function getKitsuMangaRecommendations(input: RecommenderInput): Promise<RecommendationResult> {
  ...
  const queriesToTry = buildKitsuQueries(input);
