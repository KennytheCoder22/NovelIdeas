...
function isKitsuCandidate(candidate: Candidate): boolean {
  return candidate.source === 'kitsu';
}

function teenMangaFormatFloor(targetMax: number, queryLadder: QueryLadderTerm[]): number {
...
function enforceTeenMangaRepresentation(
  ...
  const required = teenMangaFormatFloor(targetMax, queryLadder);
  if (required <= 0) return selected;

  const current = [...selected];
  const selectedKeys = new Set(current.map(identityKey));
  let mangaCount = current.filter((candidate) => isKitsuCandidate(candidate)).length;

  if (mangaCount >= required) return current;

  const kitsuCandidates = [...pool]
    .filter((candidate) => isKitsuCandidate(candidate))
    .filter((candidate) => !selectedKeys.has(identityKey(candidate)))
    .sort((left, right) => getScore(right) - getScore(left));

  if (!kitsuCandidates.length) return current;
  ...
  let injectIndex = 0;
  while (mangaCount < required && injectIndex < kitsuCandidates.length) {
    const slot = replaceableIndexes.shift();
    const replacement = kitsuCandidates[injectIndex];
    injectIndex += 1;
...
function runFinalPass(
  ...
  const credibleOnly = applySoftFilter(
    ...
    (candidate) => {
      ...
      const preserveKitsuCandidate = candidate.source === 'kitsu';

      if (preserveVisualCandidate || preserveKitsuCandidate) return false;
...
if (selected.length < targetMin) {
  remainingBackfillPool = credibleOnly.filter(
  ...
  logFinalPassStage('selectedWithTeenFormatFloor', selectedWithTeenMangaRepresentation, {
    ...
    teenMangaCount: selectedWithTeenMangaRepresentation.filter(isKitsuCandidate).length,
    teenMangaRequired: lane === 'teen' ? teenMangaFormatFloor(targetMax, queryLadder) : 0,
  });
  ...
  logFinalPassStage('finalWithTeenFormatFloor', finalWithTeenMangaRepresentation, {
    ...
    teenVisualFormatRequired: lane === 'teen' ? teenVisualFormatFloor(targetMax, queryLadder) : 0,
    teenMangaCount: finalWithTeenMangaRepresentation.filter(isKitsuCandidate).length,
    teenMangaRequired: lane === 'teen' ? teenMangaFormatFloor(targetMax, queryLadder) : 0,
  });
